#!/usr/bin/env python3
"""Serve visualization static files and POST /upload -> ../images_original/"""
...
class Handler(SimpleHTTPRequestHandler):
    def do_POST(self) -> None:
        ...
        target_dir = os.path.abspath(os.path.join(here, "..", "images_original"))
        os.makedirs(target_dir, exist_ok=True)
        target_path = os.path.join(target_dir, safe)

        with open(target_path, "wb") as f:
            f.write(file_bytes)

        rel_saved_as = posixpath.join("images_original", safe)
        self._send_json({"ok": True, "saved_as": rel_saved_as})
