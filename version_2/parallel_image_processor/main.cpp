 ...
 *     containing per-stage times, num_ranks, and the row range handled by that rank.
 *   - Each rank r also writes intermediate strip PPMs under:
 *         foo/intermediate/rank_XXX_<stage>.ppm
 *     (scatter RGB, grayscale, blur, Sobel, threshold) for visualization.
 *
 * Timing:
...
std::string padded_rank_tag(int r) {
    std::ostringstream os;
    os << "rank_" << std::setfill('0') << std::setw(3) << r;
    return os.str();
}

void save_intermediate_strip(const fs::path& output_dir, int rank, const char* stage_key, const Image& img) {
    const fs::path p =
        output_dir / "intermediate" / (padded_rank_tag(rank) + "_" + stage_key + ".ppm");
    try {
        PPMHandler::savePPM(img, p.string());
    } catch (const std::exception& e) {
        std::cerr << "[rank " << rank << "] intermediate " << stage_key << ": " << e.what() << "\n";
    }
}
...
int main(int argc, char** argv) {
    ...
    if (rank == 0) {
        try {
            fs::create_directories(output_dir / "intermediate");
        } catch (const std::exception& e) {
            std::cerr << "Failed to create intermediate directory: " << e.what() << "\n";
            MPI_Abort(MPI_COMM_WORLD, 1);
        }
    }
    MPI_Barrier(MPI_COMM_WORLD);
    ...
    save_intermediate_strip(output_dir, rank, "01_scatter_rgb", local_color);
    save_intermediate_strip(output_dir, rank, "02_grayscale", local_gray);
    ...
    save_intermediate_strip(output_dir, rank, "03_blur", blurred_local);
    ...
    save_intermediate_strip(output_dir, rank, "04_sobel", edges_local);
    ...
    save_intermediate_strip(output_dir, rank, "05_threshold", stencil_local);
    ...
    // Write per-rank metrics CSV into the same output directory.
    {
        ...
        if (mf) {
            mf << "rank,rows_start,rows_end,rows_local,num_ranks,"
